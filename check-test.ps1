...
Function Check($ip,$username,$passwd){
    ...
    echo $ip
    # 多线程运行
    $thread1 = [PowerShell]::Create().AddScript($task1)
    $thread2 = [PowerShell]::Create().AddScript($task2)
    $thread3 = [PowerShell]::Create().AddScript($task3)
    # 异步运行线程
    $job1 = $thread1.BeginInvoke()
    $job2 = $thread2.BeginInvoke()
    $job3 = $thread3.BeginInvoke()
    $thread1.EndInvoke($job1)
    $thread2.EndInvoke($job2)
    $thread3.EndInvoke($job3)
    # 释放资源
